<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Thêm Voucher Mới</h2>
    ...
    <div class="card shadow border-0 rounded-3">
        <div class="card-body p-4">
            <form action="?page=admin-voucher-store" method="POST" id="voucherForm">
                <div class="row g-4">
                        ...
                        <div class="admin-card mb-4 bg-light border-0">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="mo_ta" id="mo_ta" class="form-control" rows="4" 
                                          placeholder="Nhập mô tả về điều kiện áp dụng, chi tiết chương trình ưu đãi..."></textarea>
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="loai_giam_gia" id="loai_giam_gia" class="form-select">
                                        <option value="tien_co_dinh">Giảm tiền cố định (đ)</option>
                    ...
                    <div class="col-md-4">
                        <div class="admin-card mb-4 bg-light border-0">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="ma_hang" id="ma_hang" class="form-select">
                                    <option value="0">Tất cả mọi hạng thành viên</option>
                                    <?php foreach ($tiers as $tier): ?>
                                        <option value="<?= $tier['id'] ?>"><?= htmlspecialchars($tier['ten_hang']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="kieu_hien_thi" id="kieu_hien_thi" class="form-select">
                                    <option value="cong_khai">Công khai (Tất cả mọi người đều thấy)</option>
                                    <option value="ca_nhan">Cá nhân (Chỉ hiển thị cho người được Admin tặng)</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check form-switch pt-1">
                                    <input class="form-check-input" type="checkbox" name="trang_thai" id="trang_thai" checked value="1">
                                    <label class="form-check-label fw-semibold text-success" for="trang_thai" id="trang_thai_label">Đang kích hoạt</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-primary w-100 py-2.5 fw-bold shadow-sm">
                                <i class="bi bi-save2 me-2"></i>Lưu Voucher
                            </button>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ngày bắt đầu: bây giờ
    const localNow = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`;
    document.getElementById('ngay_bat_dau').value = localNow;
    ...
    document.getElementById('ngay_ket_thuc').value = localFuture;
